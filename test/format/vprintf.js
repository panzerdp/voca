...
describe('vprintf', function() {
  it('should return a string according to formatting', function() {
    ...
    expect(v.vprintf('%+f+%+f', [50.123456789, 0])).toBe(
      '+50.123457++0.000000'
    );
    expect(v.vprintf('%e %e %E', [100, 0, 0.1])).toBe(
  ...
  it('should throw exceptions when the formatter is not valid or not enough arguments', function() {
    expect(v.vprintf.bind(v, '%2$s %1$s', ['Alexander'])).toThrowError(
      'sprintf(): Too few arguments'
    );
    expect(v.vprintf.bind(v, '%a', ['Alexander'])).toThrowError(
      'sprintf(): Unknown type specifier'
    );
    expect(v.vprintf.bind(v, PRINTABLE_ASCII, ['Alexander'])).toThrowError(
      'sprintf(): Unknown type specifier'
    );
  ...
  it('should return an unmodified string for missing formatting specifiers', function() {
    ...
    expect(v.vprintf('Without formatting', [])).toBe('Without formatting');
    expect(v.vprintf('Without formatting', [undefined])).toBe(
      'Without formatting'
    );
