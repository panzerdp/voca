...
describe('latinise', function() {
  it('should latinise a string', function() {
    ...
    expect(v.latinise('cafe\u0301')).toBe('cafe');
    expect(v.latinise('colecção cópias críticos é tão')).toBe(
      'coleccao copias criticos e tao'
    );
    expect(v.latinise('književnošću čuvanje')).toBe('knjizevnoscu cuvanje');
    expect(v.latinise('anglikonų šiurkščios užrašinėti')).toBe(
      'anglikonu siurkscios uzrasineti'
    );
    expect(v.latinise('Schuß für Pfarrerstöchter')).toBe(
      'Schus fur Pfarrerstochter'
    );
    expect(v.latinise('publicó éxito nació María')).toBe(
      'publico exito nacio Maria'
    );
    ...
    expect(v.latinise('Wkrótce po ślubie pisarka zaszła w ciążę')).toBe(
      'Wkrotce po slubie pisarka zaszla w ciaze'
    );
    expect(
      v.latinise(
    ...
    expect(
      v.latinise('Україна розташована в південно-східній частині Європи')
